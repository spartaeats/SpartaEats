...
//    }
    ...
    public int getTime(Coordinate start, Coordinate target) {
        ...
        TmapRouteResponse response = client.post()
                ...
                .body(request)
                .retrieve()
                .toEntity(TmapRouteResponse.class)
                .getBody();

        assert response != null;
        return response.features().get(0).properties().totalTime() / 60;
    }
}