...
public class OrderService {
	...
	private BigDecimal calculateItemTotal(OrderCreateRequest request, Map<UUID, Item> itemMap,
		Map<UUID, ItemOption> optionMap) {
	...
	private List<OrderCreateResponse.ItemResponse> buildAndSaveOrderItem(Order newOrder, OrderCreateRequest request,
		Map<UUID, Item> itemMap, Map<UUID, ItemOption> itemOptionMap) {
		...
		return savedOrderItems.stream()
			.map(orderItem -> {
				...
				List<OrderItemOption> currentOptions = optionsMapByItemId.getOrDefault(orderItem.getId(),
	...
	private OrderSnapshotDto fetchOrderSnapshotDto(AddressSupplyDto addressSupplyDto, Store store,
		Map<UUID, Item> itemMap, Map<UUID, ItemOption> itemOptionMap, OrderCreateRequest request) {
		...
		BigDecimal deliveryFee = BigDecimal.valueOf(
			tmapApiClient.getDistance(addressSupplyDto.coordinate(), storeCoordinate) * 100L);
	...
	public OrderCreateResponse createOrder(User user, OrderCreateRequest request) {
		...
		List<OrderCreateResponse.ItemResponse> itemResponses = buildAndSaveOrderItem(newOrder, request, itemMap,
			itemOptionMap);
	...
	public OrderSingleResponse getOrderDetail(User user, UUID id) {
		...
		List<OrderSingleResponse.ItemResponse> itemResponses = orderItemList.stream()
			.map(orderItem -> {
				// Map에서 현재 orderItem의 ID에 해당하는 옵션 리스트를 가져옴
				List<OrderItemOption> currentOptions = optionsMapByOrderItemId.getOrDefault(orderItem.getId(),
					Collections.emptyList());
